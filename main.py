    ...
    try:
        ...
        if len(content) < 1:
            ...
            for game in content:
                ...
                time = game['time']
                print(f"Time: {time} {home_team['name']} {home_team['goals']} : {away_team['goals']} {away_team['name']}")
            print(delimeter(50))
...
def all_games(url_path):
    try:
        ...
        for game in content:
            ...
            print(f"{index}.  {home_team['name'].ljust(15, ' ')} {home_goal}  :  {away_goal} {away_team['name'].rjust(18, ' ')}")
            print(delimeter(50))
            index += 1
    except:
        print(delimeter(50))
    ...
    try:
        ...
        for group in groups:
            ...
            for country in teams:
                print(f"{index}. {country['name'].ljust(20, ' ')} {country['group_points']} | {country['wins']} | {country['draws']} | {country['losses']} | {country['goals_for']} | {country['goals_against']} | {country['goal_differential']}")
                index += 1
        print(delimeter(50))
    except:
        print('No Information Found')                  
...
def games_general_information(url_path):
    try:
        ...
        for game in content:
            ...
            away_team = game['away_team'] 
            print(delimeter(125))
            print(f"{home_team['name']} VS {away_team['name']} | Date: {date} | Location: {location} | Stadium: {stadium} | Attendance: {attendance}")
        print(delimeter(125))
    except:
        print("No Information Found.")    
